...
<nav id="nav" class="bg-white border-gray-200 dark:bg-gray-900">
  <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
    <!-- Logo Section -->
    <a href="<?= base_url('agent/dashboard') ?>" class="flex items-center space-x-3 rtl:space-x-reverse">
      <img src="<?= base_url('public/img/blue_logo.png') ?>" class="h-14" alt="Peres Imóveis Logo" />
      <span class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Área do Corretor</span>
    </a>
    ...
    <div class="flex items-center md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
      ...
      <button type="button" class="flex text-sm bg-gray-800 rounded-full md:me-0 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600" id="user-menu-button" aria-expanded="false" data-dropdown-toggle="user-dropdown" data-dropdown-placement="bottom">
        ...
        <img class="w-12 h-12 rounded-full" src="<?= base_url('public/uploads/profile_photos/' . (session()->get('profile_photo') ? esc(session()->get('profile_photo')['file_name']) : 'default.png')) ?>" alt="Agent photo">
      </button>

      <!-- Agent Dropdown Menu -->
      <div class="z-50 hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow dark:bg-gray-700 dark:divide-gray-600" id="user-dropdown">
        <div class="px-4 py-3">
          <span class="block text-sm text-gray-900 dark:text-white">Corretor</span>
        ...
        <ul class="py-2" aria-labelledby="user-menu-button">
          ...
          <li>
            <a href="<?= base_url('broker/logout') ?>" class="block px-4 py-2 text-sm text-red-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Sair</a>
          </li>
        </ul>
      </div>
    </div>
